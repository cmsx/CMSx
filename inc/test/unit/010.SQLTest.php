...
class SQLTest extends PHPUnit_Framework_TestCase
{
  function testSelectPrefix()
  {
    ...
    $sql = SQL::Select('pages p, users u')->setPrefix('yeah_')->make();
  ...
  {
    $sql = SQL::Select('pages')->where(12)->make();
  ...
  {
    $sql  = SQL::Select('pages')->where(array('some'=> 'thing'));
    ...
    $sql = SQL::Select('pages')->where('`some`="thing"', '`another`>1');
    ...
    $sql = SQL::Select('pages')->where(array('id'=> 12, 'is_active'=> 1));
    ...
    $sql = SQL::Select('pages')->where(12, true);
    $exp = 'SELECT * FROM `pages` WHERE `id`=12 AND `is_active`=1';
    $this->assertEquals($exp, $sql->make(true), 'Значения подставляются в запрос');
  }
  ...
  {
    $sql1 = SQL::Select('pages')->columns('id', '`title`', 'something');
    $sql2 = SQL::Select('pages')->columns(array('id', '`title`', 'something'));
  ...
  {
    $sql1 = SQL::Select('pages')->orderby('id', '`title`', 'something DESC');
    $sql2 = SQL::Select('pages')->orderby(array('id', '`title`', 'something DESC'));
  ...
  function testAlter()
  {
    ...
    $sql = SQL::Alter('pages')->rename('not_pages');
    ...
    $sql0 = SQL::Alter('pages')->addColumn('name', 'VARCHAR(20)');
    $sql1 = SQL::Alter('pages')->addColumn('name', 'VARCHAR(20)', true);
    $sql2 = SQL::Alter('pages')->addColumn('name', 'VARCHAR(20)', 'id');
    ...
    $sql0 = SQL::Alter('pages')->modifyColumn('id', 'INT UNSIGNED');
    $sql1 = SQL::Alter('pages')->modifyColumn('id', 'INT UNSIGNED', true);
    $sql2 = SQL::Alter('pages')->modifyColumn('id', 'INT UNSIGNED', 'title');
